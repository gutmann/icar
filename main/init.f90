...
!   allocate all arrays in domain
    subroutine domain_allocation(domain,options,nx,nz,ny,nsoil)
        ...
        if (options%block_options%block_flow) then
            ...
            domain%terrain_blocking = 0
            allocate(domain%u_block(nx+1,nz,ny))    ! store wind field generated by blocking process
            domain%u_block = 0
            allocate(domain%v_block(nx,nz,ny+1))    ! store wind field generated by blocking process
            domain%v_block = 0
            allocate(domain%w_block(nx,nz,ny))      ! store wind field generated by blocking process
            domain%w_block = 0
        endif
        ...
        domain%thstar=0
        allocate(domain%thv(nx,ny))         ! virtual potential temp on lowest model level
        domain%thv=0
        ...
        !domain%CHS2=0.01
        allocate(domain%exch_m(nx,ny))      ! exchange coefficient for momentum
        domain%exch_m=0.01
